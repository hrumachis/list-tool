...
export class LoginComponent implements OnInit {
    ...
    submit(e) {
        ...
        if (!this.busy) {
            ...
            this.Auth.login(this.username, this.password, this.port).subscribe(data => {
	...
	inputPort(e) {
		...
		if (e.key != "Backspace" && e.key != "Enter") {
			if (!e.key.match(/^[0-9]+$/)) {
				this.infoMessage("Port contain only numbers", 3, e);
			} else if (this.port.length >= 5 && !this.isAllTextSelected(this.port)) {
				this.infoMessage("Port maximum length is 5 numbers", 3, e);
			}
		}
    }
